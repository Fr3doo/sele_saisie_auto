...
_cgi_labels: list[str] = cast(
    list[str],
    _DEFAULTS.get("cgi_options", ["", "N/A", "Oui", "Non"]),
)
...
_cgi_dej_labels: list[str] = cast(
    list[str],
    _DEFAULTS.get("cgi_options_dejeuner", ["", "1"]),
)
cgi_options_dejeuner: list[CGILunchOption] = [
    CGILunchOption(label) for label in _cgi_dej_labels
]

# --- Options « Billing action » ----------------------------------------- #
_billing_map: dict[str, str] = cast(
    dict[str, str],
    _DEFAULTS.get(
...
# --- Options « Planning de travail » ------------------------------------ #
_work_schedule_labels: list[str] = cast(
    list[str],
    _DEFAULTS.get(
