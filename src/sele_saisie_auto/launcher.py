...
def tab_planning(
    ...
    container = create_a_frame(planning_tab, fill="both", expand=True)

    schedule_frame = create_a_frame(container, side="left", fill="both", expand=True)

    header = create_a_frame(schedule_frame, padding=(10, 5, 10, 5))
    create_modern_label_with_pack(header, "", side="left")
    ...
    mission_frame = create_a_frame(
        container, side="right", fill="y", expand=False, padding=(10, 10, 10, 10)
    )
    ...
    create_modern_label_with_pack(mission_frame, "Project Code:", side="top")
    create_modern_entry_with_pack(mission_frame, project_var, side="top")

    create_modern_label_with_pack(mission_frame, "Activity Code:", side="top")
    create_modern_entry_with_pack(mission_frame, activity_var, side="top")

    create_modern_label_with_pack(mission_frame, "Category Code:", side="top")
    create_modern_entry_with_pack(mission_frame, category_var, side="top")

    create_modern_label_with_pack(mission_frame, "Sub Category Code:", side="top")
    create_modern_entry_with_pack(mission_frame, sub_category_var, side="top")

    create_modern_label_with_pack(mission_frame, "Billing Action:", side="top")
    create_combobox_with_pack(
        mission_frame, billing_var, values=BILLING_LABELS, side="top"
    )

    mission_vars = {
        "project_code": project_var,
        "activity_code": activity_var,
        "category_code": category_var,
        "sub_category_code": sub_category_var,
        "billing_action": billing_var,
    }
...
def update_schedule(
    ...
    for day, (desc_var, hours_var) in schedule_vars.items():
        section[day] = f"{desc_var.get()},{hours_var.get()}"
...
def update_cgi_info(
    ...
    project_info["project_code"] = mission_vars["project_code"].get()
    project_info["activity_code"] = mission_vars["activity_code"].get()
    project_info["category_code"] = mission_vars["category_code"].get()
    project_info["sub_category_code"] = mission_vars["sub_category_code"].get()
    project_info["billing_action"] = mission_vars["billing_action"].get()
...
def write_raw_cfg(
    ...
    raw_cfg.set(
        "project_information", "project_code", mission_vars["project_code"].get()
    )
    raw_cfg.set(
        "project_information", "activity_code", mission_vars["activity_code"].get()
    )
    raw_cfg.set(
        "project_information", "category_code", mission_vars["category_code"].get()
    )
    raw_cfg.set(
        "project_information",
        "sub_category_code",
        mission_vars["sub_category_code"].get(),
    )
    raw_cfg.set(
        "project_information", "billing_action", mission_vars["billing_action"].get()
    )
    for day, vars_dict in cgi_vars.items():
        raw_cfg.set(
