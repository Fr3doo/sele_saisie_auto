...
def generer_cle_aes(TAILLE_CLE=32, log_file=None) -> bytes:
    ...
    try:
        ...
        write_log("Clé AES générée avec succès.", log_file, "DEBUG")
        return key
...
def chiffrer_donnees(donnees: str, cle: bytes, TAILLE_BLOC=128, log_file=None) -> bytes:
    ...
    try:
        ...
        write_log("Données chiffrées avec succès.", log_file, "DEBUG")
...
def stocker_en_memoire_partagee(nom: str, donnees: bytes, log_file=None):
    ...
    try:
        ...
        write_log(f"Données stockées en mémoire partagée avec le nom '{nom}'.", log_file, "DEBUG")
        return memoire
    except Exception as e:
...
def supprimer_memoire_partagee_securisee(memoire: shared_memory.SharedMemory, log_file=None):
    ...
    try:
        ...
        memoire.close()
        memoire.unlink()
        write_log("Mémoire partagée supprimée de manière sécurisée.", log_file, "INFO")
    except Exception as e:
...
def recuperer_de_memoire_partagee(nom: str, taille: int, log_file=None) -> bytes:
    ...
    try:
        ...
        write_log(f"Données récupérées depuis la mémoire partagée avec le nom '{nom}'.", log_file, "DEBUG")
...
def dechiffrer_donnees(donnees_chiffrees: bytes, cle: bytes, TAILLE_BLOC=128, log_file=None) -> str:
    ...
    try:
        ...
        write_log("Données déchiffrées avec succès.", log_file, "DEBUG")
