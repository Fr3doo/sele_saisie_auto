...
- **Description** :
  ...
  5. Avant d’ouvrir la PR :
      ...
      - exécuter `poetry run radon cc src/ -s` et `poetry run radon mi src/`. Pour produire un rapport HTML, installe d’abord le plugin [`radon-html`](https://pypi.org/project/radon-html/) puis lance `poetry run radon html src/ -o radon-report`. Les contrôles doivent réussir. Si erreur, corrige-les avant d’ouvrir la PR.
